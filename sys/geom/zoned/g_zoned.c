 ...
 * if it took ownership of bp i.e. an async chain is running or the flush has
 * failed, false if the caller should forward the flush normally.
 */
static bool
    ...
    struct bio *bp)
{
	...
	mtx_lock(&sc->sc_lock);
	dirty = sc->sc_dirty;
	dirtylo = sc->sc_dirtylo;
	dirtyhi = sc->sc_dirtyhi;
	hdrdirty = sc->sc_hdrdirty;
	/* Leave the state dirty for the next flush if nothing can carry it. */
	buf = (fc != NULL) ? g_zoned_encode_dirty(sc, &off, &total) : NULL;
	mtx_unlock(&sc->sc_lock);
	if (buf == NULL) {
		g_free(fc);
		/* Nothing to commit, so the flush stands on its own. */
		if (!dirty)
			return (false);
		/*
		 * The table stays in memory, thus this flush cannot claim it
		 * reached the medium.  Say so, rather than forwarding a flush
		 * that commits none of the state the caller is flushing for.
		 */
		G_ZONED_DEBUG(0, "No memory to commit the zone table;"
		    " failing the flush.");
		g_io_deliver(bp, G_ZONED_EXTERR(bp, ENOMEM,
		    "No memory to commit the zone table."));
		return (true);
	}

	fc->fl_dirtylo = dirtylo;
	fc->fl_dirtyhi = dirtyhi;
	fc->fl_hdrdirty = hdrdirty;
	fc->fl_orig = bp;
	fc->fl_cp = cp;
	fc->fl_sc = sc;
	fc->fl_buf = buf;
...
static void
g_zoned_punch_queue(struct g_zoned_softc *sc, struct bio_queue_head *queue,
    const struct disk_zone_rep_entry *z)
{
	struct bio *dbp;
	...
	dbp = g_new_bio();
	if (dbp == NULL) {
		G_ZONED_DEBUG(1, "No memory to reclaim zone %ju.",
		    (uint64_t)g_zoned_zoneno(sc, z->zone_start_lba));
		return;
	}
	dbp->bio_cmd = BIO_DELETE;
	dbp->bio_offset = (off_t)z->zone_start_lba * sc->sc_secsize;
	dbp->bio_length = sc->sc_zonesize;
	dbp->bio_data = NULL;
	bioq_insert_tail(queue, dbp);
}

static void
g_zoned_punch_drain(struct bio_queue_head *queue)
{
	struct bio *dbp;

	while ((dbp = bioq_takefirst(queue)) != NULL)
		g_destroy_bio(dbp);
}

static void
g_zoned_punch_issue(struct bio *bp, struct g_consumer *cp,
    struct bio_queue_head *queue)
{
	struct g_zoned_punch *pc;
	struct bio *dbp;

	pc = (bioq_first(queue) != NULL) ? g_malloc(sizeof(*pc), M_NOWAIT) :
	    NULL;
	if (pc == NULL) {
		g_zoned_punch_drain(queue);
		g_io_deliver(bp, 0);
		return;
	}
	pc->pu_orig = bp;
...
g_zoned_zonecmd(struct bio *bp, struct g_zoned_softc *sc)
{
	...
	case DISK_ZONE_RWP: {
		...
		if (all) {
			...
			if ((rwp->id % sc->sc_zonesecs) != 0) {
				g_io_deliver(bp, G_ZONED_EXTERR(bp, EINVAL,
				    "Zone ID is not the lowest LBA of a zone."
				    "  id=%ju zonesecs=%ju",
				    (uint64_t)rwp->id,
				    (uint64_t)sc->sc_zonesecs));
				return;
			}
		...
		/*
		 * A reset drops the zone's contents, thus hand them back to the
		 * backing store as long as it is open for writing.
		 */
		cp = LIST_FIRST(&bp->bio_to->geom->consumer);
		punch = args->zone_cmd == DISK_ZONE_RWP && cp->acw > 0;
		bioq_init(&punchq);
		...
		for (i = first; i < limit; i++) {
			...
			case DISK_ZONE_RWP:
				/*
				 * Empty zones have nothing to reclaim, but a
				 * reset still clears the "reset recommended"
				 * bit that a fault may have left on it.
				 */
				if (z->zone_condition == DISK_ZONE_COND_EMPTY) {
					if ((z->zone_flags &
					    DISK_ZONE_FLAG_RESET) == 0)
						continue;
				...
				if (punch)
					g_zoned_punch_queue(sc, &punchq, z);
				break;
			}
			g_zoned_mark_dirty(sc, i);
		}
		mtx_unlock(&sc->sc_lock);
		if (punch) {
			g_zoned_punch_issue(bp, cp, &punchq);
			return;
		}
		g_io_deliver(bp, 0);
		return;
...
/*
 * A write whose data never reaches the medium leaves the zone claiming blocks
 * that were never written, so retract the optimistic write-pointer advance, as
 * long as no later write has moved the pointer further.  The zone rolls back
 * to the condition the write found it in.
 */
static void
g_zoned_write_undo(struct g_zoned_softc *sc, off_t offset, off_t length,
    uint8_t oldcond)
{
	struct disk_zone_rep_entry *z;
	uint64_t lba, end;
	...
	lba = offset / sc->sc_secsize;
	end = (offset + length) / sc->sc_secsize;
...
g_zoned_write_done(struct bio *cbp)
{
	...
	if (cbp->bio_error != 0 && sc != NULL)
		g_zoned_write_undo(sc, cbp->bio_offset, cbp->bio_length,
...
g_zoned_start(struct bio *bp)
{
	...
	if (cbp == NULL) {
		if (bp->bio_cmd == BIO_WRITE)
			g_zoned_write_undo(sc, bp->bio_offset, bp->bio_length,
			    oldcond);
