...
atf_test_case reset_wp_all_readonly cleanup
reset_wp_all_readonly_head()
{
	atf_set "descr" \
	    "gzoned keeps the contents of a zone reset all did not reset"
	atf_set "require.user" "root"
}
reset_wp_all_readonly_body()
{
	gzoned_test_setup
	...
	# Fill the start of zone 1 with something unrecognizable.
	atf_check -e ignore dd if=/dev/random of=pattern bs=1m count=1
	atf_check -e ignore \
	    dd if=pattern of=/dev/${md}.zoned bs=1m oseek=256 count=1
	atf_check gzoned fault -z 1 -s ro ${md}.zoned

	atf_check zonectl -d /dev/${md}.zoned -c rwp -a
	atf_check_equal "1" "$(zone_count ro)"
	atf_check_equal "0x80800" "$(zone_wp 524288)"
	atf_check -e ignore \
	    dd if=/dev/${md}.zoned of=after bs=1m iseek=256 count=1
	atf_check cmp pattern after
}
reset_wp_all_readonly_cleanup()
{
	gzoned_test_cleanup
}

atf_test_case zone_id_lowest_lba cleanup
zone_id_lowest_lba_head()
{
	atf_set "descr" \
	    "gzoned only takes a zone ID that is a zone's lowest LBA"
	atf_set "require.user" "root"
}
zone_id_lowest_lba_body()
{
	gzoned_test_setup
	...
	atf_check zonectl -d /dev/${md}.zoned -c rwp -l 0x80000
	atf_check -s not-exit:0 -e ignore \
	    zonectl -d /dev/${md}.zoned -c rwp -l 0x80001
	atf_check -s not-exit:0 -e ignore \
	    zonectl -d /dev/${md}.zoned -c open -l 1
	atf_check -s not-exit:0 -e ignore \
	    zonectl -d /dev/${md}.zoned -c finish -l 0x7ffff
	atf_check_equal "4" "$(zone_count empty)"

	atf_check zonectl -d /dev/${md}.zoned -c rwp -a
}
zone_id_lowest_lba_cleanup()
{
	gzoned_test_cleanup
}
...
atf_test_case reset_wp_clears_fault cleanup
reset_wp_clears_fault_head()
{
	atf_set "descr" "gzoned reset write pointer clears an injected reset fault"
	atf_set "require.user" "root"
}
reset_wp_clears_fault_body()
{
	gzoned_test_setup

	zoned_attach_md

	# Freshly created zone is already empty: the injected fault is the only
	# thing a reset has to clear.
	atf_check gzoned fault -z 3 -s reset ${md}.zoned
	atf_check_equal "1" "$(zone_count reset)"
	atf_check zonectl -d /dev/${md}.zoned -c rwp -l $(zone_start 3)
	atf_check_equal "0" "$(zone_count reset)"

	# ALso holds when every zone is reset at once.
	atf_check gzoned fault -z 3 -s reset ${md}.zoned
	atf_check_equal "1" "$(zone_count reset)"
	atf_check zonectl -d /dev/${md}.zoned -c rwp -a
	atf_check_equal "0" "$(zone_count reset)"
}
reset_wp_clears_fault_cleanup()
{
	gzoned_test_cleanup
}
...
atf_init_test_cases()
{
	...
	atf_add_test_case reset_wp_all_readonly
	atf_add_test_case zone_id_lowest_lba
	...
	atf_add_test_case reset_wp_clears_fault
